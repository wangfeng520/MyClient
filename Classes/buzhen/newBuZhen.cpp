...
bool newBuZhen::init()
{
    ...
    Sprite *bkgBz=Sprite::create("zjm_bkg.png");
    bkgBz->setPosition(Point(size.width/2, size.height/2));
    this->addChild(bkgBz);
    Sprite* bj = Sprite::create("bz_bkg.png");
    bj->setPosition(Point(size.width/2, size.height/2));
    this->addChild(bj);
    ...
    for (int i = 0; i < 19; i++) {
        ...
        if(i<9)
        {
            item->setPosition(Point(82+(i%3)*115,  -115 * (i/3) + 640));
        }
        else
        {
            item->setPosition(Point(((size.width - kk*5)/8 + kk) * ((i-9)%5 + 1) -kk+13,   -75 * ((i-9)/5) + 293));
...
bool newBuZhen::onTouchBegan(cocos2d::Touch *touch, cocos2d::Event *event)
{
...
//                    Pointoint p =Point(touchPoint.x + 10 ,touchPoint.y + 10);
//                    
...
//    istouch=btnsMenu->TouchBegan(touch, event);
...
void newBuZhen::onTouchMoved(cocos2d::Touch *touch, cocos2d::Event *event)
{
...
//    if(istouch){
//        
//        btnsMenu->TouchMoved(touch, event);
//    }
}
void newBuZhen::onTouchEnded(cocos2d::Touch *touch, cocos2d::Event *event)
{
    Point touchPoint=convertTouchToNodeSpace(touch);
...
//    if (istouch) {
//        btnsMenu->TouchEnded(touch, event);
//        
//        istouch=false;
//        
//    }
...
//    rapidjson:: strbuf;
//    rapidjson::Writer<rapidjson::StringStream> writer(strbuf);
//    sendMessage.Accept(writer);
//
