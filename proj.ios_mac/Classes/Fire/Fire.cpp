...
void Fire::whetherChat(){
    ...
    this->addChild(enemyIcon,10);
    enemyName1=Label::create("1", "Marker Felt", 35, Size(180, 45), TextHAlignment::LEFT);
    enemyName1->setAnchorPoint(Point(0, 0));
    //create(na1, "Marker Felt", 35);
    enemyName1->setVisible(true);
    enemyName1->setAnchorPoint(Point(0, 0));
    enemyName1->setPosition(Point(80, 140));
    enemyName1->setColor(Color3B(230, 0, 18));
    this->addChild(enemyName1,11);
    
    
    //        sprintf(icc, "s_wj_icon_%d.png",ic);
    int sex = 1;// CData::getCData()->getSex();
    if(sex == 1)
    {
   ...
   // playerIcon->setScale(0.38);
    ...
    this->addChild(playerIcon,10);
    
    
    playerName1=Label::create("s", "Marker Felt", 35, Size(180, 45), TextHAlignment::LEFT);
...
void Fire::MoreAttack(){//******************************
    ...
    if(_eneChanHp[moreene[ea]]+_eneCurrHp[moreene[ea]]<=0){
        ...
        CallFunc * func = CallFunc::create(CC_CALLBACK_0(Fire::removeAction, this));
...
void Fire::moreAttackName(){
    ...
    label->setTag(999);
    this->addChild(label);
    ActionInterval *action=MoveTo::create(0.88, Point(size.width/2, size.height/2));
    FadeOut *fadeout=FadeOut::create(0.12f);
    CallFunc * func1=CallFunc::create(CC_CALLBACK_0(Fire::moreAttackTitle, this));
...
void Fire::debuff(int eneBid,int enechaid,int changeHp1,int eneCurrHp,int eneTotalHp){
    ...
    //ActionInterval *a_jianxue=MoveTo::create(0.45, Point(reduB->getPosition().x, reduB->getPosition().y+80));
...
void Fire::AttackName(){
    ...
    if(!strcmp(skillname, "")){
        ...
        ActionInterval *action=MoveTo::create(0.88, Point(size.width/2, size.height/2));
        FadeOut *fadeout=FadeOut::create(0.12f);
        CallFunc * func1=CallFunc::create(CC_CALLBACK_0(Fire::whethertwice, this));//Fire::attacked
        // CallFunc * fun3=CallFunc::create(this, callfunc_selector(Fire::removeSprite));
        FiniteTimeAction *allaction=Sequence::create(action,fadeout,func1,NULL);
        label->runAction(allaction);
        this->whethertwice();
...
void Fire::moreAttackTitle(){
    ...
    ActionInterval * moveto=MoveTo::create(0.7, Point(player->getPosition().x, player->getPosition().y));//enemy
    CallFunc * fun2=CallFunc::create(CC_CALLBACK_0(Fire::moreAttacked, this));
...
void Fire::moreAttacked(){
    ...
    for (int i=0; i<_length; i++) {
        ...
        LabelAtlas * reduB=LabelAtlas::create(changeBlood, "number.png", 30, 50, '-');
        reduB->setColor(Color3B(255, 0, 0));
        reduB->setTag(9800+i);
        reduB->setAnchorPoint(Point(0.5, 0));
        reduB->setPosition(Point(enemy->getContentSize().width/2, enemy->getContentSize().height/2));
        ...
        //ActionInterval *a_jianxue1=MoveTo::create(0.2, Point(reduB->getPosition().x, reduB->getPosition().y+60));
        FadeOut *a_jiaxue_fadeout1=FadeOut::create(0.5f);
        CallFunc*  func2=CallFunc::create(CC_CALLBACK_0(Fire::MoreAttack, this));
...
void Fire::actionAttack(int eneBattleId,int eneChaID,int eneChangeHp,int eneCurrentHp,int eneTotalHp,int actionID,int chaBattleID,int chaId,int skillId){
    ...
    if (kaishi==0){
        ...
        {
            SpriteFrame *spf=SpriteFrameCache::getInstance()->getSpriteFrameByName(String::createWithFormat("%02d.png",i)->getCString());
            // log(String::createWithFormat("%02d.png",i)->getCString());
            log("\n");
            kneif.pushBack(spf);
        }
...
void Fire::fangzhao(){
    ...
    if(_actionId==98){
        ...
        texiao->setPosition(Point(player->getPosition().x, player->getPosition().y));
        this->addChild(texiao,5);
        ActionInterval * moveto=MoveTo::create(1/Time, Point(player->getPosition().x, player->getPosition().y));//enemy
...
void Fire::jsonData(char *json){
    ...
    //    d2.Parse<rapidjson::kParseDefaultFlags>(json);
    //
    //    //    if (read.parse(json, value)) {
    //    std::string out=d2["message"].GetString();
    //    cout<<out<<endl;
    //
    //     rapidjson:: Value root(kArrayType);
    //     root = d2;
    //    rapidjson::Value &data=root["data"];
    //
    ////    rapidjson::Value setData=data.operator[]("setData");//["setData"];
    ////    log("%s",json);
    ...
    //        name=startValue[i]["chaName"].GetString();
    //        nengliang=startValue[i]["chaCurrentPower"].GetInt();
    //        chaTotalHp=startValue[i]["chaTotalHp"].GetDouble();
    //        chaCurrentHp=startValue[i]["chaCurrentHp"].GetDouble();
